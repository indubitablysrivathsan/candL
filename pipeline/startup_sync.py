...
============================
Manifest (CSV) tracks download state only.
Processing state is checked directly from nse.db via is_processed().
"""
...
def run_startup_sync():
    ...
    for trade_date in sorted(all_dates):
        try:
            # Check DB directly — source of truth for what's processed
            ...
            print(f"Processing {trade_date} (missing: {', '.join(missing)}) ...")

            process_trade_date(trade_date)

            # Re-check DB after processing and update manifest flags
